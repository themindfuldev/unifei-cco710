...
public class CopyClient
{
   public static void main(String[] args)
   {
      ...
      try
      {
         ...
         // Obtém o caminho do servidor.
         String server = args[0];

         // Obtém a porta.
         int servPort = (args.length == 4) ? Integer.parseInt(args[3]) : 7;

         // Obtém os nomes dos arquivos Origem e Destino.
         String nameSource = args[1];
         String nameDestiny = args[2];

         // Posição corrente do arquvo.
         int position = 0;

         // Itera enquanto não houver erro, nem finalizar o processo.
         do
         {
            // Cria um socket para a comunicação e obtém os streams de E/S.
            ...
            switch (resultRead)
            {
               ...
               default:
                  ...
                  // Cria um socket para a comunicação e obtém os streams de
                  // E/S.
                  socket = new Socket(server, servPort);
                  in = new DataInputStream(socket.getInputStream());
                  out = new DataOutputStream(socket.getOutputStream());
